    ...
    for (const auto r : rows)
    {
        for (const auto sr : r->subrows)
        {
            ...
            while (last_cluster)
            {
                ...
                last_cluster->x = floor(last_cluster->x / sr->site_width + 0.5) * sr->site_width;
                int clsx = last_cluster->x;
...
void abacus(){
    ...
    for(auto c : cells){
        ...
        while(down_row_idx >= 0 && abs(c->g_y - rows[down_row_idx]->y) < best_cost) {
            ...
            auto [sr,cost] = rows[down_row_idx]->place_row_trial(c,max_dis);
        ...
        while(up_row_idx < (int)rows.size() && abs(c->g_y - rows[up_row_idx]->y) < best_cost){
            auto [sr,cost] = rows[up_row_idx]->place_row_trial(c,max_dis);
            // double cost = c->get_dis();
            if(cost < best_cost) {
                best_cost = cost;
                best_subrow = sr;
