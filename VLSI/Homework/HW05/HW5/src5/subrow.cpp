...
void subrow::add_cell_in_cluster(Cluster::ptr cls, cell* i){
...
void subrow::merge_cluster(Cluster::ptr c1, Cluster::ptr c2){
    c1->cells.insert(c1->cells.end(), c2->cells.begin(), c2->cells.end());
    c1->last_c = c2->last_c;
    c1->weight += c2->weight;
    c1->q_c += c2->q_c - c2->weight*c1->width;
    c1->width += c2->width;
...
void subrow::collapse(Cluster::ptr  cur_cluster){
    ...
    Cluster::ptr prev = cur_cluster->pre;
...
// void subrow::add_cluster_in_subrow(Cluster::ptr cls){
//     last_cls = cls;
...
// void subrow::remove_cluster(Cluster::ptr  target) {
//     target->cells.clear();
//     target->first_c = nullptr;
//     target->last_c=nullptr;
//     clusters.remove_if([&](Cluster::ptr c) {
//         if (c == target) {
//             // 移除调试输出
//             // delete c;
//             return true;
...
void subrow::Place_subrow(cell* c){
    if(!last_cls || last_cls->x + last_cls->width <= c->g_x){
        auto cur_cls = Cluster::ptr(new Cluster(c->g_x, y, last_cls,c->max_d_after_place(1000,site_width)));
        // cur_cls->subr = this;
        // cur_cls->first_c = c;
