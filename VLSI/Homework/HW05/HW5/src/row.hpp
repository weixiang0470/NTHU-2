...
public:
    ...
    int getSiteX(double nonalignX,double (*func)(double) = std::round){
        double shiftX = nonalignX - this->x;
        return this->x + func(shiftX / s_w) * s_w;
    }
    ...
    std::pair<subrow*,double> place_row_trial(cell* c,int max_dis,int round){
        ...
        if(last_cluster == nullptr || last_cluster->x + last_cluster->width <= opt_x) {
            ...
            std::stack<Cluster::ptr> c_stk;
            while(1){
                // to check whether all cluster can be push forward
                c_stk.push(last_cluster);
                c_x = c_q/c_weight;
            ...
            if (round){
                // if(std::ceil(std::abs(last_cluster->q_c/last_cluster->weight - std::max(sr->min_x,(int) c_q/c_weight))/s_w) > max_move){
                //         std::cout<<"c_q = "<<std::max(sr->min_x,(int)c_q)<<", last_cluster->q_c="<<last_cluster->q_c<<", x = "<<std::ceil(std::abs(last_cluster->q_c - std::max(sr->min_x,(int)c_q))/s_w)<<", max_move="<<max_move<<std::endl;
                //         // throw;
                //         return {nullptr,std::numeric_limits<double>::max()};
    ...
    Cluster::ptr collapse(Cluster::ptr cluster, subrow* sr){
        cluster -> x = cluster->q_c / cluster-> weight;
        if(cluster->x < sr->min_x) cluster->x = sr->min_x;
        else if(cluster->x > sr->max_x - cluster->width) cluster->x = sr->max_x - cluster->width;
        
        Cluster::ptr pre_cluster = cluster->pre;
        if(pre_cluster!=nullptr && pre_cluster->x + pre_cluster->width > cluster->x) {
            // collapse with the previous cluster
            pre_cluster->cells.insert(pre_cluster->cells.end(), cluster->cells.begin(), cluster->cells.end());
            pre_cluster->weight += cluster->weight;
            pre_cluster->q_c += cluster->q_c - cluster->weight * pre_cluster->width;
            pre_cluster->width += cluster->width;
            return collapse(pre_cluster,sr);
        }else return cluster;
    ...
    void place_row_final(cell* c, subrow* sr, int max_dis){
        ...
        if(!last_cluster || last_cluster->x + last_cluster->width <= opt_x) {
            ...
            sr->last_cls = collapse(last_cluster,sr); // update the last cluster  
        }
