...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        Outfile = argv[2];
        if(strstr(argv[1], "public2.txt") != NULL){cooling_rate=0.998;num_nighour=5;}
        else {num_nighour=4;}
        construct_all_nodes();
        // return 0;
        // for(auto& n : nodes){
...
BStarTree<int64_t> SA(){
    ...
    set_coord(bestTree.root);
    OutputFile(Outfile, bestarea);
...
//op5
void move_rotate_node(Node<int64_t>*& root){
    vector<Node<int64_t>*> allNodes;
    traverse(root, allNodes);
...
BStarTree<int64_t> get_neighbour(const BStarTree<int64_t>& original) {
    ...
    int op = get_random_index(0, num_nighour);
    // cout<<"op="<<op<<endl;  num_nighour
    switch(op) {
        case 0: 
        ...
        case 3:
            ...
            break;
        case 4:
            move_rotate_node(new_tree.root);
            break;
    }
