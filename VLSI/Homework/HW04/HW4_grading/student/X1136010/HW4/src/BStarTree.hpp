...
struct Node
{
    ...
    Node() : x(0), y(0), width(0), height(0),symaxis(0), lchild(nullptr), rchild(nullptr),parent(nullptr), rotate(false), is_sg(false) {}
...
class BStarTree
{
    ...
    Node<T> *buildTree(const std::vector<Node<T> *> &preorder, const std::vector<Node<T> *> &inorder, size_t &i, int64_t l, int64_t r,Node<T>* pr)
    {
        ...
        node->parent = pr;
        node->lchild = buildTree(preorder, inorder, i, l, idx - 1, node);
        node->rchild = buildTree(preorder, inorder, i, idx + 1, r, node);
        return node;
    }
...
public:
    ...
    void buildTree(const std::vector<Node<T> *> &preorder, const std::vector<Node<T> *> &inorder)
    {
        ...
        size_t i = 0;
        root = buildTree(preorder, inorder, i, 0LL, n - 1, nullptr);
    }
