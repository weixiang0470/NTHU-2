...
Chromosome Selection(uniform_int_distribution<>& dis,const vector<Chromosome>& new_populations){
    // int best = dis(g);
...
void next_gen(uniform_int_distribution<>& dis){
    ...
    while(new_populations.size()<(population_size*1.5)){
        Chromosome p1 = Selection(dis,new_populations);
        Chromosome p2 = Selection(dis,new_populations);
        ...
        // }
        // cout<<"new populations size = "<<new_populations.size()<<" p-size"<< population_size * 1.5<<endl;
    }
